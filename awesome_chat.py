...
SERVER_PATH = Path().cwd().joinpath("hugginggpt/server/")
config_path = SERVER_PATH.joinpath("configs/config.lite.yaml")
...
parse_task_demos_or_presteps = open(SERVER_PATH.joinpath(config["demos_or_presteps"]["parse_task"]), "r").read()
choose_model_demos_or_presteps = open(SERVER_PATH.joinpath(config["demos_or_presteps"]["choose_model"]), "r").read()
response_results_demos_or_presteps = open(SERVER_PATH.joinpath(config["demos_or_presteps"]["response_results"]), "r").read()
...
MODELS = [json.loads(line) for line in open(SERVER_PATH.joinpath("data/p0_models.jsonl"), "r").readlines()]
MODELS_MAP = {}
for model in MODELS:
    tag = model["task"]
...
def send_request(data):
    ...
    if api_type == "openai":
        ...
        HEADER = None
    response = requests.post(api_endpoint, json=data, headers=HEADER, proxies=PROXY, verify=False)
...
def huggingface_model_inference(model_id, data, task):
    ...
    task_url = f"https://api-inference.hf-mirror.com/models/{model_id}"
