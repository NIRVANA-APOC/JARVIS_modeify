...
for model in "${models[@]}"; do
    ...
    echo "----- Downloading from https://hf-mirror.com//${model} -----"
	if [ -d "${model}" ]; then
		(cd "${model}" && git pull && git lfs pull)
	else
		# git clone --recurse-submodules "https://huggingface.co/${model}" "${model}"
        git clone --recurse-submodules "https://hf-mirror.com//${model}" "${model}"
	fi
done
...
for dataset in "${datasets[@]}"; do
    ...
    echo "----- Downloading from https://hf-mirror.com//datasets/${dataset} -----"
	if [ -d "${dataset}" ]; then
		(cd "${dataset}" && git pull && git lfs pull)
	else
		# git clone --recurse-submodules "https://huggingface.co/datasets/${dataset}" "${dataset}"
        git clone --recurse-submodules "https://hf-mirror.com//datasets/${dataset}" "${dataset}"
	fi
done